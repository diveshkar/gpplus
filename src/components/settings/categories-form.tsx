...
function CategoryRow({ category }: { category: Category }) {
  ...
  function remove() {
    startDelete(async () => {
      await deletePaintType(category.id);
      router.refresh();
  ...
  return (
    <form action={formAction} className="flex flex-col gap-3 py-4">
      <input type="hidden" name="paint_type_id" value={category.id} />
...
/**
 * Manage the business's earning categories: rename, change rates, add, or
 * remove. Each category sets how many points a sale earns for that kind of
 * product or service.
 */
